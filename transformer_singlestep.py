...
def get_data(file):
    ...
    # series = read_csv('/home/dl/ren/transformer-prediction/daily-min-temperatures.csv', header=0, index_col=0, parse_dates=True, squeeze=True)
    print(len(series))
    sampels = 2500
    ...
    amplitude_train = scaler_train.fit_transform(series.values.reshape(-1, 1)).reshape(-1)
    amplitude_test = scaler_test.fit_transform(series.values.reshape(-1, 1)).reshape(-1)

    # amplitude = scaler.fit_transform(dataset.reshape(-1, 1)).reshape(-1)
    #print(amplitude)

    # convert our train data into a pytorch train tensor
    #train_tensor = torch.FloatTensor(train_data).view(-1)
    # todo: add comment.. 
    train_sequence = create_inout_sequences(amplitude_train,input_window)
    ...
    test_data = create_inout_sequences(amplitude_test,input_window)
...
def train(train_data):
    ...
    for batch, i in enumerate(range(0, len(train_data) - 1, batch_size)):
        ...
        print("input's shape is:", data.shape)
...
file = '/home/dl/ren/transformer-prediction/0724_60s.csv'
train_data, val_data= get_data(file)
...
#torch.save(best_model.state_dict(), 'best_model_Transformer.pth')
